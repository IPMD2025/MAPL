...
from utils.util import mkdir_if_missing
...
def visualize_ranked_results(
    ...
    for q_idx in range(num_q):
        qimg_path, qpid,qclothid, qcamid = query[q_idx][:4]
        qimg_path_name = qimg_path[0] if isinstance(
            qimg_path, (tuple, list)
        ) else qimg_path
        
        qimg_path = qimg_path.replace("/Pad_datasets", "")
        qimg_path =qimg_path[:-4]+'.png'
        ...
        for g_idx in indices[q_idx, :]:
            gimg_path, gpid,gclothid,gcamid = gallery[g_idx][:4]
            # invalid = (qpid == gpid) & (qcamid == gcamid)
            invalid = (qpid == gpid) & (qcamid == gcamid) | (qpid == gpid) & (qclothid == gclothid)
            gimg_path = gimg_path.replace("/Pad_datasets", "")
            gimg_path =gimg_path[:-4]+'.png'
